...
class _DriverHomeScreenState extends State<DriverHomeScreen>
  ...
  Future<void> _acceptTrip(BusStop stop) async {
    ...
    // Set route and clear selected bus stop
    setState(() {
      _currentRoute = route_models.Route.fromJson(routeResult['data']);
    ...
    // Set destination and trigger UI update with error handling
    try {
      setState(() {
  ...
  Future<void> _onMapCreated(MapboxMap mapboxMap) async {
    ...
    try {
      // Create the PointAnnotationManager to manage markers
      pointAnnotationManager =
          await mapboxMap.annotations.createPointAnnotationManager();

      // Load marker icon as raw bytes from assets
      final ByteData byteData =
          await rootBundle.load('assets/images/bus_stop.png');
      final Uint8List imageData = byteData.buffer.asUint8List();

      // Define the geographic coordinates where to place the marker
      final Point point = Point(
          coordinates:
              Position(-74.00913, 40.75183)); // Replace with your coords

      // Configure the annotation options (position and icon)
      ...
      // Add the marker to the map through the manager
      await pointAnnotationManager?.create(options);

      print("✅ Added custom marker to the map");
      setState(() {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Failed to add marker: $e')),
      );
      setState(() {
    ...
    // Optional: update location puck settings
  ...
  Widget build(BuildContext context) {
    ...
    if (_selectedBusStop != null) {
      ...
      bottomSheetChild =
          const SizedBox.shrink(); // Hide bottom sheet if no trip or bus stop
    }
      ...
      body: Stack(
        children: [
          ...
          if (_selectedBusStop != null || _currentDestination != null)
            Positioned(
              bottom: safePadding +
                  56.0, // Account for bottom nav bar and safe area
              left: 16.0, // Position at bottom left
              child: SlideTransition(
                ...
                child: AnimatedContainer(
                  ...
                  width: _isTripCardMinimized
                      ? 300.0 // Fixed width for minimized card
                      : MediaQuery.of(context).size.width -
                          32.0, // Full width for non-minimized
                  height: _isTripCardMinimized
                      ? 80.0 // Fixed small height for minimized
                      : screenHeight * 0.5,
                  decoration: BoxDecoration(
                    ...
                    // border: Border.all(color: Colors.red, width: 2.0), // Uncomment for debugging
                  ),
  ...
  Widget _buildMinimizedTripCard() {
    ...
    return GestureDetector(
      ...
      child: Card(
        elevation: 8.0, // Increased for stronger floating effect
        shape: RoundedRectangleBorder(
          borderRadius:
              BorderRadius.circular(16.0), // Rounded for creative look
        ),
        margin: const EdgeInsets.all(8.0), // Spacing to feel floating
        child: Container(
          decoration: BoxDecoration(
